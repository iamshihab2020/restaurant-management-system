...
import { useDashboardStats, useSalesReport, usePopularItems } from "@/lib/hooks/use-reports";
import {
  Area,
  AreaChart,
  Bar,
...
export default function ReportsPage() {
  ...
  // React Query hooks for dashboard stats
  const { data: dashboardStats, isLoading: statsLoading } = useDashboardStats();
  const { data: salesData, isLoading: salesLoading } = useSalesReport();
  const { data: popularItemsFromAPI, isLoading: itemsLoading } = usePopularItems();

  const isLoading = statsLoading || salesLoading || itemsLoading;

  // Use real data from API if available, fallback to empty array
