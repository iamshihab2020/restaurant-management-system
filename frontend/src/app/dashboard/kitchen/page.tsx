...
import { useKitchenOrders, useStartOrder, useMarkItemReady, useCompleteOrder } from "@/lib/hooks/use-kitchen";
...
export default function KitchenPage() {
  ...
  const markItemReadyMutation = useMarkItemReady();
  ...
  const markItemReady = async (orderId: string, itemId: string) => {
    try {
      await markItemReadyMutation.mutateAsync({ orderId, itemId });
  ...
  const startPreparingOrder = async (orderId: string) => {
    try {
      await startOrderMutation.mutateAsync(orderId);
  ...
  /**
   * Bump order to served status
   */
  const bumpOrder = async (orderId: string) => {
    try {
      await updateStatusMutation.mutateAsync({ id: orderId, status: "served" });
                    ...
                    <div className="flex items-start justify-between mb-2">
                      ...
                      {item.status === "ready" ? (
                        ...
                        <Button
                          size="sm"
                          variant="outline"
                          onClick={() => markItemReady(order._id, item.id)}
                ...
                <div className="flex gap-2 pt-2 border-t">
                  {order.status === "confirmed" && (
                    <Button
                      ...
                      onClick={() => startPreparingOrder(order._id)}
                    >
