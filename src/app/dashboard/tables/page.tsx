...
export default function TablesPage() {
  ...
  const getTableOrder = (tableId: string) => {
    return activeOrders.find((order) => order.tableId === tableId);
  };
  ...
  const handleStatusChange = (tableId: string, newStatus: TableStatus) => {
    setTables(
      tables.map((table) =>
        table.id === tableId
          ? { ...table, status: newStatus, updatedAt: new Date() }
          : table
      )
    );
    toast({
      title: "Table Status Updated",
      description: `Table ${tables.find((t) => t.id === tableId)?.tableNumber} is now ${newStatus}`,
    });
      ...
      {viewMode === "grid" ? (
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
          {filteredTables.map((table) => {
            ...
            const duration = getOrderDuration(table.id);
            const total = getOrderTotal(table.id);
                  ...
                  <Card className="border-primary/20 bg-primary/5">
                    ...
                    <CardContent className="space-y-3">
                      ...
                      <div className="flex justify-between">
                        ...
                        <span className="font-semibold">{getTableOrder(selectedTable.id)?.items.length}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-muted-foreground">Total:</span>
                        <span className="font-bold text-primary text-xl">
                          ${getOrderTotal(selectedTable.id).toFixed(2)}
                        </span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-muted-foreground">Duration:</span>
                        <span className="font-semibold">{getOrderDuration(selectedTable.id)} minutes</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-muted-foreground">Order Status:</span>
                        <Badge variant="outline" className="capitalize">
                          {getTableOrder(selectedTable.id)?.status.replace("_", " ")}
                        </Badge>
                      </div>
                    </CardContent>
                  </Card>
