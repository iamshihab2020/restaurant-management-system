...
export default function PaymentsPage() {
  ...
  const processPayment = () => {
    ...
    setPaymentHistory([paymentRecord, ...paymentHistory]);

    toast({
      title: "Payment Processed",
      description: `${selectedOrder.orderNumber} paid via ${paymentMethod.replace("_", " ")}`,
    });
  ...
  const processRefund = () => {
    ...
    setPaymentHistory(
      paymentHistory.map((p) =>
        p.id === refundOrder.id ? { ...p, amount: -p.amount } : p
      )
    );

    toast({
      title: "Refund Processed",
      description: `Refunded $${refundOrder.amount.toFixed(2)} for ${refundOrder.orderNumber}`,
    });
